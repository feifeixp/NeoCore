...
class TDPWorldGenerator:
    ...
    def add_dimension_axis(self, module: str, name: str, options: List[Dict[str, str]], weights: List[float] = None):
        """添加维度轴
    ...
    def _extract_all_options(self, config):
        ...
        for module, module_data in config.items():
            if module in self.modules:
                options[module] = []
                for axis in module_data.get("axes", []):
                    options[module].append([o["name"] if isinstance(o, dict) else o for o in axis.get("options", [])])
        return options
...
"""
    ...
    def _get_mixed_description(self, axis):
        ...
        description = f"■ {name}: {options[dominant_era]['name'] if isinstance(options[dominant_era], dict) else options[dominant_era]}"
...
class CharacterDNAGenerator:
    ...
    def __init__(self, tdp_world):
        """初始化角色DNA生成器
        
        Args:
            tdp_world: TDP世界生成器实例
        """
        self.tdp_world = tdp_world
        self.protocol_version = tdp_world.protocol_version
        self.era_factors = {
            "ancient": {
    ...
    def generate_character(self, birth_datetime, gender, era="modern"):
        """生成角色
        ...
        character = {
            "metadata": {
                ...
                "version": "3.0",
                "soul_id": soul_id,
                "universe_id": self.tdp_world.universe_id
            },
            "basic": {
...
{character_data.get("background", {}).get("story", "暂无背景故事")}"""
    ...
    def _get_alternative_era(self, current_era, offset=1):
        """获取替代纪元"""
        eras = ["ancient", "modern", "future"]
        current_index = eras.index(current_era)
        return eras[(current_index + offset) % len(eras)]
    
    def _generate_era_specific_description(self, name, era, abilities, events):
        """生成特定纪元下的角色描述"""
        # 根据能力和事件生成描述
        main_ability = abilities["main"]
        ability_values = abilities["values"]
        
        # 挑选高能力值
        high_ability = max(ability_values.items(), key=lambda x: x[1])
        high_ability_name = "战斗" if high_ability[0] == "attack" else "防御" if high_ability[0] == "defense" else "智慧"
        
        # 生成描述模板
        templates = {
            "ancient": [
                f"{name}是一位{main_ability}，以{high_ability_name}见长。",
                f"自幼{events[0]['description'] if events else '便展现出非凡天赋'}，{events[1]['description'] if len(events) > 1 else '踏上修行之路'}。",
                f"擅长{abilities['special'][0] if abilities['special'] else '运用五行之力'}，在修真界小有名气。"
            ],
            "modern": [
                f"{name}是一位资深{main_ability}，{high_ability_name}水平一流。",
                f"早年{events[0]['description'] if events else '受过专业训练'}，后来{events[1]['description'] if len(events) > 1 else '成为行业专家'}。",
                f"专注于{abilities['special'][0] if abilities['special'] else '技术创新'}，获得业界广泛认可。"
            ],
            "future": [
                f"{name}是一位量子时代的{main_ability}，{high_ability_name}指数达到惊人水平。",
                f"在神经网络初始化时{events[0]['description'] if events else '展现出适配天赋'}，后经{events[1]['description'] if len(events) > 1 else '多次系统升级'}。",
                f"精通{abilities['special'][0] if abilities['special'] else '跨维度数据操作'}，是赛博空间的传奇存在。"
            ]
        }
        
        return "\n".join(templates[era])
    
    def _generate_alternative_era_description(self, name, abilities, original_era, target_era):
        """生成替代纪元下的形态描述"""
        # 转换能力名称
        ability_conversions = {
            "ancient": {
                "剑修": "精密机械师" if target_era == "modern" else "量子武器专家",
                "药师": "药剂师" if target_era == "modern" else "基因工程师",
                "阵法师": "系统架构师" if target_era == "modern" else "空间构建师",
                "丹道师": "化学家" if target_era == "modern" else "能量转换专家",
                "符箓师": "密码学家" if target_era == "modern" else "量子编码师"
            },
            "modern": {
                "金融分析师": "聚财道人" if target_era == "ancient" else "量子经济预测师",
                "生物学家": "草药师" if target_era == "ancient" else "基因设计师",
                "软件工程师": "阵法师" if target_era == "ancient" else "神经网络架构师",
                "能源专家": "炼丹师" if target_era == "ancient" else "能量转换工程师",
                "建筑师": "风水师" if target_era == "ancient" else "空间构建师"
            },
            "future": {
                "量子计算专家": "推演大师" if target_era == "ancient" else "算法架构师",
                "生态系统设计师": "仙植师" if target_era == "ancient" else "环境工程师",
                "数据心理学家": "心象师" if target_era == "ancient" else "行为分析师",
                "能源转化工程师": "丹炉师" if target_era == "ancient" else "核能专家",
                "星际建筑师": "洞天师" if target_era == "ancient" else "超级建筑师"
            }
        }
        
        # 获取能力转换
        main_ability = abilities["main"]
        converted_ability = ability_conversions.get(original_era, {}).get(main_ability, main_ability)
        
        # 生成描述模板
        templates = {
            "ancient": f"如果转世到上古修真时代，{name}将成为一位{converted_ability}，凭借与生俱来的灵气感知和悟性，在修真界独辟蹊径。",
            "modern": f"若生于现代社会，{name}会是一名杰出的{converted_ability}，利用分析能力和专业知识解决复杂问题，在业界获得声誉。",
            "future": f"在赛博未来，{name}将以{converted_ability}的身份，融合量子技术与意识增强，成为跨维度数据空间的开拓者。"
        }
        
        return templates[target_era]

# 示例使用
def main():
    # 创建一个三纪元世界生成器
    generator = TDPWorldGenerator(protocol_version="1.1")
    
    # 添加维度轴
    # 添加地理维度
    generator.add_dimension_axis(
        module="geo",
        name="大陆形态",
        options=[
            "灵脉浮岛（上古）",
    ...
    # 设置规则
    generator.set_rule("cross_era_interaction", True)
    generator.set_rule("entropy_constraint", 0.78)
    
    # 生成世界
    world = generator.generate_world()
    
    # 生成世界描述
    world_description = generator.generate_world_description()
    print(world_description)
    
    # 导出世界配置
    generator.export_to_yaml("tri_era_universe.yaml")
    
    # 创建角色生成器
    character_generator = CharacterDNAGenerator(generator)
    
    # 生成角色
    from datetime import datetime
    character = character_generator.generate_character(
        birth_datetime=datetime(2000, 2, 20, 8, 28),
        gender="male",
        era="modern"
    )
    
    # 生成角色描述
    character_description = character_generator.generate_character_description(character)
